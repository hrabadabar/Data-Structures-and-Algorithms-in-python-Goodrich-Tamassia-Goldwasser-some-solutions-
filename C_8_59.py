...
def LCA(T,p,q):
	...
	if T.parent(p) == q:				
		...
		the_not_chosen = p if d1 < d2 else q  # the 
		# loop until c
...
def diameter(T,p):
	"""Returns the diameter of a binary tree T with positions p and q"""
	if T.left(p) and T.right(p):
		...
		while not queue2.is_empty():
			node = queue2.dequeue()
			stack2.append(node)
			for j in T.children(node):
				queue2.enqueue(j)
				
	elif T.left(p) is None and T.right(p) is not None:
		return diameter(T,T.right(p))
		
	elif T.left(p) and not T.right(p):
		return diameter(T,T.left(p))
		
	else:
		return 0
	
	lowest_ans = LCA(T,stack[-1],stack2[-1])

	return T.depth(stack[-1]) + T.depth(stack2[-1]) - T.depth(lowest_ans)
		
		
		
	
print(diameter(n,n.root()))
